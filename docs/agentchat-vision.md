...
| **v1.2.0.dev28** | Reply-to-message (threaded chat) | `POST /v1/ui/post` accepts optional `reply_to: {id, author, snippet}` — server adds NIP-29 `["e", id, "", "reply"]` tag, dedups the parent author into `#p` (so an LLM agent will respond), and emits a non-standard `["parent_snippet", text]` tag (truncated 140ch, newlines flattened) for instant quoted-block rendering without a relay round-trip. New endpoint `GET /v1/ui/messages/{id}` fetches a single kind:9 event from the relay (POST then GET fallback) — used by the chat UI when the user clicks a "Replying to: …" rail but the parent has scrolled off. UI: hover-revealed "Reply" button per message, reply context bar above the composer with × cancel, "Replying to: …" rail above reply bubbles that scrolls-to-parent (or pops a modal if off-screen). `reply_to` + `parent_snippet` now surface on the SSE message envelope so the client doesn't have to re-parse tags. 23 new unit tests; full suite 697/697. | 697 |
